...
test("clients read their own storm data through the Founder-mode idiom", async () => {
  const migration = await readFile(migrationUrl, "utf8");
  // 5 = warnings SELECT, dismissals SELECT, dismissals INSERT, and both the
  // USING and WITH CHECK sides of the dismissals UPDATE policy.
  assert.equal(
  ...
  assert.match(migration, /"Clients dismiss own storm warnings"/);
  assert.match(migration, /"Clients update own storm warning dismissals"/);
});
...
test("clients can never write audit rows directly", async () => {
  const migration = await readFile(migrationUrl, "utf8");
  assert.match(migration, /GRANT SELECT ON public\.client_storm_warnings TO authenticated;/);
  assert.doesNotMatch(migration, /GRANT [^;]*INSERT[^;]* ON public\.client_storm_warnings/);
  assert.match(migration, /GRANT SELECT, INSERT, UPDATE ON public\.client_storm_warning_dismissals TO authenticated;/);
  assert.doesNotMatch(migration, /GRANT [^;]*DELETE[^;]* ON public\.client_storm_warning/);
});
