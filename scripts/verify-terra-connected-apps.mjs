...
try {
  ...
  for (const viewport of [
    ...
    const garminButton = page.getByRole("button", { name: /preview sync|connect/i }).first();
    ...
    const whoopSection = page
      .getByRole("heading", { name: "WHOOP and Strava" })
      .locator("xpath=ancestor::section");
    if (!await whoopSection.getByRole("button").isDisabled()) {
      throw new Error(`${viewport.name}: WHOOP/Strava should remain unavailable`);
    }
