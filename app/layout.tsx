...
export const metadata: Metadata = {
  ...
  verification: {
    google: "gEfBGCVWoPvBI4Ve28VclWmte_mrzZPUtOPwBTPhpuA",
  },
  alternates: {
    canonical: siteUrl,
  },
      ...
      <body className={`${inter.variable} ${barlowCondensed.variable} antialiased`}>
        ...
        <Script
          ...
          dangerouslySetInnerHTML={{
            __html: `var isNative=window.Capacitor&&window.Capacitor.isNativePlatform&&window.Capacitor.isNativePlatform();var isLocal=/^(localhost|127\\.0\\.0\\.1|\\[::1\\])$/.test(window.location.hostname);if(!isNative&&!isLocal&&'serviceWorker' in navigator){var registerServiceWorker=function(){navigator.serviceWorker.register('/sw.js').then(function(reg){if(!reg)return;reg.update();reg.addEventListener('updatefound',function(){var worker=reg.installing;if(!worker)return;worker.addEventListener('statechange',function(){if(worker.state==='installed'&&navigator.serviceWorker.controller){worker.postMessage({type:'SKIP_WAITING'});}});});}).catch(function(){});};var refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',function(){if(refreshing)return;refreshing=true;window.location.reload();});if(document.readyState==='complete'){registerServiceWorker();}else{window.addEventListener('load',registerServiceWorker,{once:true});}}window.__pwaInstallPrompt=null;if(!isNative){window.addEventListener('beforeinstallprompt',function(e){e.preventDefault();window.__pwaInstallPrompt=e});}`,
          }}
        />
      </body>
    </html>
