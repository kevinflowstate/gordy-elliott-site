...
function analyseValues(entries: Record<string, string[]>): AnalysedValue[] {
  ...
  // Group by shared words (theme detection)
  const groups: {
    entries: { text: string; qId: number }[];
...
function QuestionCard({
  ...
  return (
    <div className="flex flex-col min-h-[60vh] justify-center max-w-xl mx-auto px-4">
      ...
      <h2 className="text-xl sm:text-2xl font-heading font-bold text-text-primary leading-tight mb-3">
        {question.prompt}
      </h2>
      ...
      <div className="space-y-4">
        {[0, 1, 2].map((i) => (
          <div key={i} className="relative">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center text-[10px] text-accent font-bold">
              {i + 1}
            </div>
            <input
              ref={refs[i]}
              ...
              placeholder={
                i === 0 ? "First thing that comes to mind..." : i === 1 ? "Second..." : "Third..."
              }
      ...
      <div className="flex items-center justify-between mt-8">
        <button
          onClick={onBack}
          className={`text-sm text-text-muted hover:text-text-secondary transition-colors cursor-pointer ${index === 0 ? "invisible" : ""}`}
        >
          Back
        </button>
        <button
          onClick={onNext}
          disabled={!canGoNext}
          className="px-6 py-2.5 bg-accent text-white text-sm font-semibold rounded-xl disabled:opacity-30 disabled:cursor-not-allowed hover:bg-accent/90 transition-all cursor-pointer"
        >
...
function AlignmentBadge({ score }: { score: number }) {
  const config = score >= 50
    ...
    : score >= 30
    ? { label: "Room to grow", desc: "Some values are dominant in specific areas but not across the board. There's room for your edges.", color: "#EF9F27", bg: "rgba(239,159,39,0.1)", border: "rgba(239,159,39,0.3)" }
    : { label: "Priorities scattered", desc: "Your priorities may need focusing. That's not a problem — it's the starting point.", color: "#2424B4", bg: "rgba(36,36,180,0.1)", border: "rgba(36,36,180,0.3)" };
...
function ResultsView({
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="text-3xl sm:text-4xl font-heading font-bold text-text-primary text-center mb-2">
        YOUR VALUES HIERARCHY
      </h1>
      <p className="text-sm text-text-muted text-center mb-8">
        Based on the evidence of your 39 answers — not assumptions, not what you think should matter. This is what actually drives you.
      </p>
      ...
      <div className="space-y-3 mb-8">
        {top3.map((v) => (
          <div
            key={v.rank}
            className="bg-bg-card border border-[rgba(224,64,208,0.15)] rounded-2xl overflow-hidden"
          >
            <button
              onClick={() => toggleExpand(v.rank)}
              className="w-full flex items-center gap-4 p-5 text-left cursor-pointer hover:bg-[rgba(224,64,208,0.03)] transition-colors"
            >
              <div className="w-10 h-10 rounded-xl bg-accent/15 border border-accent/25 flex items-center justify-center text-accent font-bold text-lg flex-shrink-0">
                {v.rank}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-heading font-bold text-text-primary text-base">{v.label}</div>
                <div className="text-xs text-text-muted mt-0.5">
                  Present in {v.breadth} of 13 areas
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="hidden sm:flex gap-1">
                  {QUESTIONS.map((q) => (
                    <div
                      key={q.id}
                      className={`w-2 h-2 rounded-full ${v.questionIds.includes(q.id) ? "bg-accent" : "bg-[rgba(255,255,255,0.06)]"}`}
                    />
                  ))}
                </div>
                <svg
                  className={`w-4 h-4 text-text-muted transition-transform ${expanded.has(v.rank) ? "rotate-180" : ""}`}
              ...
              <div className="border-t border-[rgba(255,255,255,0.04)] p-5 space-y-4">
                <div>
                  <div className="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-2">
                    EVIDENCE TRAIL — WHICH AREAS THIS SHOWED UP
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {v.questionIds.map((qId) => (
                      <span key={qId} className="text-[10px] px-2 py-1 rounded-full bg-accent/10 text-accent border border-accent/20">
                        {QUESTION_MAP[qId]?.short}
                      </span>
                    ))}
                  </div>
                </div>
        ...
        <>
          ...
          <div className="space-y-2 mb-8">
            {rest.map((v) => (
              <div
                key={v.rank}
                className="bg-bg-card/60 border border-[rgba(255,255,255,0.04)] rounded-xl p-4 flex items-center gap-3"
              >
                <div className="w-7 h-7 rounded-lg bg-[rgba(255,255,255,0.04)] flex items-center justify-center text-text-muted text-xs font-bold">
                  {v.rank}
                </div>
                <div className="flex-1 min-w-0">
      ...
      <div className="bg-bg-card border border-[rgba(255,255,255,0.06)] rounded-2xl p-6 mb-8">
        ...
        <div className="text-sm text-text-secondary leading-relaxed space-y-3">
          <p>
            Your top values aren&apos;t about finding out something new — they&apos;re about confirming what&apos;s already running in the background. No procrastination, no accountability issues, because these are the things you&apos;d do regardless.
          </p>
          <p>
            If a goal doesn&apos;t line up with these, it&apos;ll probably slip. Not because you&apos;re lazy — because it&apos;s not wired into who you are. Yet.
          </p>
          <p>
            Your programme is built around these. We map training, nutrition, and mindset against your values so that effort feels conscious, not forced.
          </p>
        </div>
      </div>
      ...
      <div className="text-center space-y-4 mb-8">
        ...
        <p className="text-sm text-text-muted">
          We&apos;ll map these against your goals in your next session. Screenshot your results or come back here any time.
        </p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            href="/portal/training"
            className="px-6 py-3 bg-accent text-white text-sm font-semibold rounded-xl no-underline hover:bg-accent/90 transition-all inline-block"
          >
            Back to Education Hub
          </Link>
          <button
            onClick={onRestart}
            className="px-6 py-3 bg-bg-card border border-[rgba(255,255,255,0.08)] text-text-secondary text-sm font-medium rounded-xl hover:border-accent/30 transition-all cursor-pointer"
          >
            Retake Assessment
          </button>
        </div>
        {saving && (
          <p className="text-xs text-text-muted">Saving your results...</p>
        )}
      </div>
    </div>
  );
...
type Phase = "intro" | "questions" | "analysing" | "results";
...
export default function ValuesDiscovery() {
  ...
  useEffect(() => {
    fetch("/api/portal/values-determination")
      ...
      .then((data) => {
        if (data?.results) setHasExisting(true);
      })
      .catch(() => {});
  }, []);
  ...
  function updateAnswer(qId: number, index: number, value: string) {
    setAnswers((prev) => {
      ...
      updated[qId][index] = value;
      return updated;
    });
  }

  function canProgress(): boolean {
    const a = answers[QUESTIONS[currentQ].id];
    return a[0].trim().length > 0;
  }
  ...
  async function runAnalysis() {
    ...
    const avg = top3.length > 0
      ? top3.reduce((s, v) => s + v.breadth, 0) / (top3.length * 13)
      : 0;
  ...
  if (phase === "intro") {
    return (
      <div className="max-w-xl mx-auto px-4 py-8">
        <Link
          href="/portal/training"
          className="text-text-muted text-sm hover:text-text-secondary transition-colors no-underline inline-flex items-center gap-1 mb-8"
        >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-heading font-bold text-text-primary mb-3">Values Determination</h1>
        ...
        <div className="bg-bg-card border border-[rgba(255,255,255,0.06)] rounded-2xl p-6 mb-6">
          <h2 className="font-heading font-bold text-text-primary mb-3">Before you start</h2>
          <div className="text-sm text-text-secondary leading-relaxed space-y-3">
            <p>
              This isn&apos;t a personality quiz. It&apos;s an evidence-based process that reveals what actually drives you — not what you think should drive you.
            </p>
            <p>
              13 questions. 3 answers each. The tool cross-references all 39 answers to find recurring themes and ranks them into a values hierarchy.
            </p>
            <p>
              Be honest. Write the first things that come to mind. There are no wrong answers — only real ones.
            </p>
          </div>
        </div>

        <div className="bg-bg-card/60 border border-[rgba(255,255,255,0.04)] rounded-2xl p-5 mb-8">
          <div className="flex items-center gap-4 text-xs text-text-muted">
            ...
            <div className="flex items-center gap-1.5">
              ...
              13 questions, 3 answers each
            </div>
          </div>
        </div>

        {hasExisting && (
          <div className="bg-accent/5 border border-accent/15 rounded-xl p-4 mb-6 text-center">
            <p className="text-xs text-text-muted">You&apos;ve completed this before. Starting again will save a new set of results.</p>
          </div>
        )}
        ...
        <div className="text-center">
          <button
            ...
            className="px-8 py-3.5 bg-accent text-white font-semibold rounded-xl hover:bg-accent/90 transition-all text-sm cursor-pointer"
          >
            Start Discovery
          </button>
        </div>
