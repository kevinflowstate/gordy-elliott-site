...
Consent mechanism as implemented: the consultation form includes an unticked-by-default checkbox stating the form may include health, training, nutrition, injury and cycle-related information and how Gordy will use it. The server rejects submission without it and stores `privacy_consent`, `privacy_consent_version` (`health_cycle_v1`) and a timestamp (`app/api/portal/consultation/route.ts`). The Terra connection screen separately requires an unticked explicit-health-data checkbox and stores `wearable_connection_v1` plus its acceptance time on the provider connection. Withdrawing consent is effected by disabling the feature, disconnecting the provider, or deleting the account.

Gap to close before launch (action plan): Terra now has connection-point wording and a versioned acceptance record; the calendar connection screen still needs its equivalent notice.
...
| R6 | Consent shortfall for special category data at optional connection points | Medium x Medium | Versioned, timestamped consultation consent exists (`health_cycle_v1`). Terra now requires a separate unticked explicit notice and records `wearable_connection_v1` with its timestamp before generating the provider widget; unsigned or unconsented webhook mappings are not accepted | Low for Terra after deployment; calendar notice remains an action |
...
| # | Action | Owner | When |
| --- | --- | --- | --- |
| 1 | Add connection-point consent/notice wording to the calendar and wearable connect screens; bump the consent version | Build team | **Terra completed in activation branch 3 August 2026; calendar remains** |
