...
| R6 | Consent shortfall for special category data at optional connection points | Medium x Medium | Versioned, timestamped consultation consent exists (`health_cycle_v1`). Terra now requires a separate unticked explicit notice that names Terra and links its end-user privacy policy, and records `wearable_connection_v2` with its timestamp before generating the provider widget; unsigned or unconsented webhook mappings are not accepted | Low for Terra after deployment; calendar notice remains an action |
