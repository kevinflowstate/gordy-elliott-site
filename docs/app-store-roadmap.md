...
## Current position - 26 August 2026

- App Store Connect record `6805066999` is named **AT CAPACITY by Gordy** under Gordy's Apple developer account.
- Registered bundle ID: `com.gordyelliott.atcapacity` on Apple team `5NU9323724`.
- Build 10 passed archive and Organizer server validation, processed successfully in TestFlight, was installed internally and is selected for version 1.0.
...
- App Store Connect now contains the free worldwide schedule, iPhone-only availability, product metadata, review account, Build 10, manual release, six screenshots, published privacy label, 16+ age override and non-medical-device declaration.
- Google OAuth verification is approved. Production contains connected Google and Outlook accounts with real synced events; exact Build 10 native-return testing remains.
- Version 1.0 remains **Prepare for Submission** and **Add for Review has not been clicked**.
...
- [x] Archive and validate Build 10 with Organizer.
- [x] Upload Build 10 to the internal TestFlight group, confirm processing and select it for version 1.0 without adding the version for review.
...
- Universal Link authentication callback validation on exact TestFlight Build 10.
...
- Completed physical-device TestFlight, APNs, accessibility and crash evidence for Build 10 after the final hosted-portal deployment.
