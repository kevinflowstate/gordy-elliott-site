...
| Direct messages, including optional voice notes and photos | Client and coach | Coaching communication | `inbox_messages`; private `inbox-audio` and `inbox-images` storage buckets | Life of account | Account deletion removes the message rows and client-folder media objects | Supabase |
...
| Wearable connection records (provider, Terra user ID, reference ID, scopes, raw Terra user object) | Terra auth webhook after client connects a provider | Manage the connection | `client_wearable_connections` | Life of account | Disconnect and account deletion call Terra deauthentication before the local record is disconnected or removed; account deletion fails safely if revocation cannot be confirmed | Supabase, Terra |
| Wearable raw webhook events (full provider payload plus idempotency hash) | Terra webhooks | Idempotency, audit, re-normalisation | `client_wearable_events` | Purged after 90 days by default (configurable from 7-365 days) | Scheduled purge or account deletion | Supabase, Terra |
